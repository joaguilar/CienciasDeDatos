...
* Evaluar la confianza del modelo priori con el fin de validar que la confianza de las reglas seleccionadas tenga al menos un 90% de que exista la relación entre las variables seleccionadas.
...
dataset2 <- dataset
dataset2$Tiempo_Respuesta <- NULL
dataset2$Edad <- NULL
head(dataset2)

#transacciones <- col_concat(dataset,sep=",")
transaccionesv2<-as(dataset2,Class = "transactions")
head(transaccionesv2)
 
 # se guardan los datos como archivo csv en formato transaccion
write(transaccionesv2,'transaccionesv2.csv',sep=',')
#write.csv(transacciones,'transaccionesv2.csv',quote = FALSE)
...
```{r}
 # se generarn las reglas usando el algoritmo a priori, con un soporte del 1% y una confianza de 0.9 o 90%
  ...
  reglasv2<- apriori(transaccionesv2,parameter = list(supp=0.001,conf=0.9,maxlen=10))
  ...
  # se filtran las reglas con confianza superior a 0.25
  mejoresReglasv2<- reglasFinalv2[quality(reglasFinalv2)$confidence>0.90]
  
```

```{r warning=FALSE}
# se leerá el archivo como transacciones formato basket
 transaccionesv2 <- read.transactions('transaccionesv2.csv',sep=',',format = 'basket')
