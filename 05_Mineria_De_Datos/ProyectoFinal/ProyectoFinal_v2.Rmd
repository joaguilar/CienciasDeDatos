...
*	Identificar al menos 3 segmentos de mercados minoristas a partir de las ventas.
...
Como se muestra en los graficos anteriores, tanto el método de Silhoutte como el del Codo indican que el numero de clusters optimo es 3. El método de estadística de gap indica que este número sería 1. Para la creación de los clusters, vamos a utilizar 3, dado que no tiene sentido para los objetivos utilizar solo 1. Esta decisión se refuerza con el hecho que dos de los tres métodos retornan este valor.
...
* Clientes de bajo valor: Corresponden a los clientes del cluster 1, que tienen pocas compras, de un monto bajo, y no compran recientemente.

* Clientes de valor medio: Corresponden a los clientes del cluster 3, que tienen una cantidad media de compras, compran recientemente, y montos de compras medios

* Clientes de alto valor: Corresponden a los clientes del cluster 2, que tienen una cantidad alta de compras, recientes, y de montos más altos
...
export_df<-export_df %>% 
  mutate(kmeans = replace(kmeans, kmeans == 2, "Valor Medio")) %>%
  mutate(kmeans = replace(kmeans, kmeans == 3, "Alto Valor")) %>%
  mutate(kmeans = replace(kmeans, kmeans == 1, "Bajo Valor")) %>%
  mutate(kmedoids = replace(kmedoids, kmedoids == 3, "Valor Medio")) %>%
  mutate(kmedoids = replace(kmedoids, kmedoids == 2, "Alto Valor")) %>%
  mutate(kmedoids = replace(kmedoids, kmedoids == 1, "Bajo Valor"))

write.csv(export_df,"resultado_cluster.csv")


centroides2<-centroides %>% 
  mutate(cluster = replace(cluster, cluster == "2", "Valor Medio")) %>%
  mutate(cluster = replace(cluster, cluster == "3", "Alto Valor")) %>%
  mutate(cluster = replace(cluster, cluster == "1", "Bajo Valor"))




medoids_df2 <- medoids_df %>%
  mutate(cluster = replace(cluster, cluster == "3", "Valor Medio")) %>%
  mutate(cluster = replace(cluster, cluster == "2", "Alto Valor")) %>%
  mutate(cluster = replace(cluster, cluster == "1", "Bajo Valor"))
...
Para la construccion de los modelos de regresion se trabajara con el campo InvoiceDay y los totales de ventas, a fin de realizar la agregacion de las ventas de forma mensual y a partir de estos datos generar otras variables que no sean de utilidad para la prediccion. 
...
plot(x = compras$Fecha, y = compras$ventas, 
     main = 'Ventas Mensuales', 
     xlab = 'Fecha', 
     ylab = 'Ventas totales mensuales', type="l")

```

El siguiente paso a ejecutar es comparar la diferencia en ventas de u mes a otro.

```{r warning=FALSE}
compras_diferencia <- compras
compras_diferencia['ventas_previas'] <- shift(compras_diferencia$ventas,n=1,fill=0)
na.omit(compras_diferencia)
...
head(compras_diferencia)

```

Al analizar el comportamiento de la diferencia de las ventas a lo largo del tiempo se observa
...
```{r}
plot(x = compras_diferencia$Fecha, y = compras_diferencia$diferencia, 
     main = 'Ventas Mensuales', 
     xlab = 'Fecha', 
     ylab = 'Ventas totales mensuales', type="l")

```
Posterior a esto se procede a crear las variables lag, las cuales nos permiten representar como columnas las ventas y el comportamiento de las mismas durante el periodo de un anno.

```{r}
for (mes in 1:13){
  field_name = paste('lag_',toString(mes))
  compras_diferencia[field_name] <- shift(compras_diferencia$diferencia,n=mes,fill=0)
}

colnames(compras_diferencia) <- c('Fecha', 'Ventas', 'Ventas_previas', 'diff', 'Dic_Anno1','Ene_Anno2','Feb_Anno2','Mar_Anno2','Abr_Anno2','May_Anno2','Jun_Anno2','Jul_Anno2','Ago_Anno2','Sep_Anno2','Oct_Anno2','Nov_Anno2','Dic_Anno2')
```
...
```
```{r}
pairs(entrenamiento[4:15])
```


Se utiliza la funcion cor para identificar la correlacion entre las variables de ventas de los diferentes meses de forma numérica.

```{r}
cor(entrenamiento[4:15])
```
...
modelo.simple <- lm(diff ~ Dic_Anno1, data = entrenamiento)
...
sqrt(mean((entrenamiento$diff - entrenamiento$Prediccion)^2))
...
#sqrt(mean((prueba$diff - prueba$Prediccion)^2))
...
A fin de reforzar los resultados del modelo de regresion lineal, se crea un modelo de regresion multiple con las variables creadas con la diferencia de las ventas de los 13 meses analizados.
...
Como se observa anteriormente los resultados del modelo0 muestran como las variables mas significativas son las variables de los 6 primeros meses. Por lo que se creara un modelo 2 para comparar los resultados.

Por lo que se procede a crear un segundo modelo unicamente con estas variables
...
hist(modelo.multiple0$residuals,
     main= 'Distribución de residuos Modelo de Regresión Múltiple 0',
     xlab = 'Residuos',
     ylab = 'Observaciones',
     breaks = 50)
````
...
plot(x = entrenamiento$diff,
     ...
     main = 'Residuos del Modelo de Regresión Múltiple 0',
     xlab = 'Valor',
     ylab = 'Residuos')
````
...
##### Modelo de regresión lineal múltiple 1 (Modelo con 6 variables)

Por lo que se procede a crear un segundo modelo únicamente con las variables con valores significativos mostrados por el modelo 0

````{r}
#Para la creación del modelo 1 se empezará por un modelo "Con las variables significativas" 
modelo.multiple1 <- lm(diff ~ Dic_Anno1 + Ene_Anno2 + Feb_Anno2 + Mar_Anno2 + Abr_Anno2 + May_Anno2 , data = entrenamiento[,4:15])
# ver el detalle del resumen
...
````{r}
#Distribución de los residuos del modelo basado en el peso
hist(modelo.multiple1$residuals,
     main= 'Residuos del Modelo de Regresión Múltiple 1',
     xlab = 'Residuos',
     ylab = 'Observaciones',
     breaks = 50)
````
...
```{r}
#Calcular el RMSE en Entrenamiento
entrenamiento$Prediccion <- predict(modelo.multiple1)
sqrt(mean((entrenamiento$diff - entrenamiento$Prediccion)^2))

# predecir valores de mpg.mpg en el conjunto de datos de prueba
prueba$Prediccion <- predict(modelo.multiple1, newdata = prueba[,4:15])
#Calcular el RMSE en prueba
#sqrt(mean((prueba$diff - prueba$Prediccion)^2))
