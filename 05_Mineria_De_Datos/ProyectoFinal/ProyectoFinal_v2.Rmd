...
TODO - GAP falla y el codo puede darnos 6 tambien.
...
Para la construccion de los modelos de regresion se trabajara con el campo InvoiceDay y los totales de ventas, a fin de realizar la agregacion de las ventas por la fecha de realización de las compras y poder monitorear el comportamiento mensual de las mismas y a partir de estos datos generar otras variables que no sean de utilidad para la predicción. 
...
```{r}
plot(x = compras_diferencia$Fecha, y = compras_diferencia$diferencia,  main = 'Ventas Mensuales',  xlab = 'Fecha',  ylab = 'Ventas totales mensuales', type="l")
```
...
Se utiliza la función pairs para analizar la correlación de las 6 primeras variables lag y su correlación con la diferencia de las ventas

```{r}
pairs(entrenamiento[4:10])
```


Se utiliza la funcion cor para identificar la correlacion entre las 12 variables lag y su correspondiente relación con la fecha precedente.
...
````{r}
#Distribución de los residuos del modelo basado en la diferencia de los montos de compras
...
Como se observa anteriormente los resultados del modelo0 muestran como las variables mas significativas son las variables de los 6 fechas subsecuentes. 

###### Evaluación Modelo Regresión Múltiple 1

````{r}
#Distribución de los residuos del modelo basado en los montos de compras
...
# predecir valores de la diferencia de compras
...
Como se puede apreciar en los gráficos a continuación la predición de las ventas se comporta de manera muy similar a las ventas historias, de igual manera se observa que el modelo se comporta de forma muy similar al comportamiento de regresión lineal simple.
