...
Al analizar el comportamiento de las ventas a lo largo del tiempo se observa que el comportamiento de las mismas pareciera tener un comportamiento similar a lo largo de todo el año, y se observa un incremento en las ventas a fin de mes.
...
Podemos ver que el sigma es de `r sigma(modelo.simple)/mean(prueba$Ventas)` para el modelo de regresión múltiple.
...
p <- ggplot() +
  ...
  geom_point()+
  labs(x="Mes de Compra")+
  ggtitle("Comparación de Ventas Mensuales Reales vs Predicción (Regresión Simple)")+
...
##### Modelo de regresión lineal múltiple 0 (Modelo con las 6 variables)

A fin de reforzar los resultados del modelo de regresion lineal, se crea un modelo de regresion múltiple con las variables creadas con la diferencia de las ventas de 7 meses predecesoras a las compras realizadas.
...
p <- ggplot() +
  ...
  geom_point()+
  labs(x="Mes de Compra")+
  ggtitle("Comparación de Ventas Mensuales Reales vs Predicción (Regresión Múltiple)")+
  scale_colour_manual("",labels = c("Ventas", "Predicción"), values = c("blue", "red")) 
p
...
Podemos ver que el sigma es de `r sigma(modelo.multiple0)/mean(prueba$Ventas)` para el modelo de regresión múltiple.
...
resultado_regresion$Test <- as.Date(resultado_regresion$Fecha, format = '%Y%m%d')
