...
Para efectos de este trabajo solo se utilizaran datos del pais "United Kingdom"

```{r warning=FALSE}
datos_uk <- retail[retail$Country == 'United Kingdom',]
summary(datos_uk)
```
...
```{r warning=FALSE}
kable(head(datos_uk[datos_uk$UnitPrice < 0,],10),"html", booktabs = T)%>%
  kable_styling(latex_options = "striped")
```
...
```{r warning=FALSE}
retail <- datos_uk[!(datos_uk$UnitPrice < 0),]
kable(head(datos_uk[datos_uk$UnitPrice < 0,],10),"html", booktabs = T)%>%
  kable_styling(latex_options = "striped")
...
```{r warning=FALSE}
kable(head(datos_uk[datos_uk$Quantity < 0,]),"html", booktabs = T)%>%
  kable_styling(latex_options = "striped")
datos_uk <- datos_uk[!(datos_uk$Quantity < 0),]
kable(head(datos_uk[datos_uk$Quantity < 0,],10),"html", booktabs = T)%>%
  kable_styling(latex_options = "striped")
```
...
```{r warning=FALSE}
datos_uk <- datos_uk %>% drop_na(CustomerID, Description)
```
...
datos <- merge(datos, visitas_por_cliente, by="CustomerID")
